...
async function main(): Promise<void> {
  await throwOnce(); //  invokes the readOnce function
  await throwSeveral();
  await throwChained();
}

try {
  await main(); //  this will wrap the function calls all into one single try/catch block
} catch (error) {
  console.log('Main Error:', (error as Error).message);
}
